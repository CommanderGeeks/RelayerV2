...
exports.decodeConfigAccount = void 0;
 ...
 */
const decodeConfigAccount = (buffer) => {
    // skip offset 8 for anchor's reserved space
    let offset = 8;
    // 1. Decode Fixed Fields in the Exact Order
    const relayer = new web3_js_1.PublicKey(buffer.slice(offset, offset + 32));
    offset += 32;
    const feeTo = new web3_js_1.PublicKey(buffer.slice(offset, offset + 32));
    offset += 32;
    ...
    for (let i = 0; i < tokenCount; i++) {
        const mint = new web3_js_1.PublicKey(buffer.slice(offset, offset + 32));
        offset += 32;
